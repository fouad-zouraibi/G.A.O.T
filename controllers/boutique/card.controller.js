...
module.exports.update = (req) => {
    return new Promise((resolve, reject) => {
        let body = req.body;
        if(body){
            cardModel.findOneAndUpdate({_id:req.params.id}, body)
            .then((res) =>{
                resolve({
                    response: true,
...
module.exports.delete = (req) => {
    return new Promise((resolve,reject) => {
        ...
        cardModel.findOneAndDelete({_id:req.params.id}, function (err, data) {
            if (err){
                reject({
                    response: false,
                    message: 'error',
                ...
                resolve({
                    response: data != null ,
                    message: data != null ? 'Deleted card' : 'card doesnt exist',
                    data: data
                });
            }
        });
...
module.exports.get = (id) => {
    return new Promise((resolve,reject) => {
        cardModel.findOne({_id:id}).then((produit)=>{
            condition = false;
            if(produit)
                condition = true;
            resolve({
