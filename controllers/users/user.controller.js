        ...
        if (user) {
            userModel.findOne({ email: user.email }).then(async (data) => {
                if (data) {
                    ...
                    if (user.password) await userDB.setPassword(user.password)
            ...
            function (err, data) {
                if (err) {
                    ...
                    resolve({
                        response: data != null,
                        message:
                            data != null ? 'Deleted User' : 'User doesnt exist',
                        data: data,
                    })
                }
            }
    ...
    return new Promise((resolve, reject) => {
        userModel
            ...
            .then(async (user) => {
                ...
                user.new_date = await moment(user.date_de_naissance).format(
                    'YYYY-MM-DD'
                )
        ...
        if (user) {
            userModel.findOne({ email: user.email }).then(async (userData) => {
                if (userData) {
                    ...
                    token =
                        valid == true ? await generateAccessToken(userData) : ''
                    resolve({
                        response: valid,
                        token: token,
                        message: valid
            ...
            userModel.findOne({ email: user.email }).then(async (data) => {
                if (!data) {
                    ...
                    async function checkCode() {
                        ...
                        let findCode = await passwordModel.findOne({
                            code: code,
                        })
                        if (findCode) {
                            checkCode()
                    ...
                    let objEmail = {
                        ...
                        subject: 'Subject',
                        html:
                            '<h1>Hello Test send Email for password:' +
                            codeV +
                            '</h1>',
                    }
